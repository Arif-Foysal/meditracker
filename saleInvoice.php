...
<?php
  ...
  $purchaseinvID=null;
    $sql = "SELECT *
        ...
        WHERE EmpID = $empID AND status = 'draft' AND type='sale'";
...
if(isset($_POST['addtoinv'])){
    ...
    $url="onsale.php?purchaseinvID=" . $purchaseinvID;
    header('Location: ' . $url);
...
WHERE o.purchaseInvoiceID = $purchaseinvID;
  ...
  if($purchaseinvID){
    if(isset($_POST['save'])){
        ...
        //update storeddrug TODO pending
...
WHERE o.purchaseInvoiceID = $purchaseinvID;
    ...
    // loop and update stock drugs quantity for every drugid
    ...
    if (mysqli_num_rows($resulty) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($resulty)) {
            ...
            $id=$row['drugID'];
            $qty=$row['drugQuantity'];

            //run update query
            $query = " UPDATE storedDrug
    ...
    VALUES ('sale',CURDATE(),'draft', $empID);
    ";
